      ...
      <div className="relative z-10 max-w-6xl mx-auto">
        <div className="text-center mb-10">
          ...
          <a
            ...
            className="text-accent font-semibold text-sm hover:text-[#00b86a] dark:hover:text-[#4dffa0] transition-colors"
          >
            @{instagramUrl.split("/").filter(Boolean).pop()}
          </a>
        </div>

        {/* 6-photo grid using real shrine images */}
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 mb-8">
          {FEED_ITEMS.map((item, i) => (
            <div
              key={i}
              className="group relative aspect-square rounded-2xl overflow-hidden cursor-pointer hover:ring-2 hover:ring-[#00b86a]/50 transition-all"
            >
              <Image
                src={item.src}
        ...
        <div className="text-center">
          <a
            ...
            target="_blank"
            rel="noopener noreferrer"
            className="inline-flex items-center gap-2 border border-[#00b86a]/40 text-accent hover:bg-[#00b86a]/10 font-semibold px-8 py-3 rounded-full transition-all"
          >
