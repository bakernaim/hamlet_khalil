      ...
      <div className="relative z-10 max-w-6xl mx-auto">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
          {testimonials.map((t, i) => (
            <div
              key={i}
              className="flex flex-col rounded-2xl p-5 sm:p-6 border border-[#d9e3dd] hover:border-[#00b86a]/35 transition-colors bg-white"
            >
              {/* Stars */}
              ...
              <p className="text-[#3d4b44] text-sm leading-7 flex-1 mb-5">
                {isRTL ? t.quote.ar : t.quote.en}
              </p>

              {/* Reviewer */}
              <div className="flex items-center gap-3 border-t border-[#d9e3dd] pt-4">
                <div className="w-9 h-9 rounded-full bg-[#00b86a]/12 border border-[#00b86a]/25 flex items-center justify-center text-[#00995a] font-bold text-sm shrink-0">
                  {(isRTL ? t.name.ar : t.name.en).charAt(0)}
                </div>
                <div>
                  {/* Name — Cairo (small UI text) */}
                  <div className="text-[#0c1a14] font-semibold text-sm">
                    {isRTL ? t.name.ar : t.name.en}
                  </div>
                  <div className="text-[#00995a]/80 text-xs mt-0.5">
