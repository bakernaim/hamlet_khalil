...
export default function ZiyaratForm({ id }: { id?: string }) {
  ...
  useEffect(() => {
    ...
    let active = true;
    (async () => {
      const res = await fetch(`/api/admin/ziyarat/${id}`, { cache: "no-store" });
      if (!active) return;
      if (!res.ok) {
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    setSaving(true);
    setFormError("");
    const res = await fetch(id ? `/api/admin/ziyarat/${id}` : "/api/admin/ziyarat", {
      method: id ? "PUT" : "POST",
  ...
  return (
    <div className="flex flex-col h-[calc(100dvh-5.5rem)] sm:h-[calc(100dvh-6.5rem)] lg:h-[calc(100dvh-4rem)]">
      <header className="shrink-0 mb-4">
        <Link href="/admin/ziyarat" className="text-ink/45 hover:text-ink text-sm inline-flex items-center gap-1 mb-3">
          ← Back to Ziyarat Packages
        </Link>
        <h1 className="text-2xl font-bold text-ink">{editing ? "Edit Package" : "New Package"}</h1>
        <p className="text-ink/45 text-sm mt-1">Pilgrimage package shown on the homepage.</p>
      </header>
        ...
        <form onSubmit={onSubmit} className="flex flex-col flex-1 min-h-0">
          ...
          <Field label="Info / details (EN)" hint="Shown in the “Details” view on the site — visitors can also listen to it">
            <RichTextEditor
              ...
              placeholder="Full information about this Ziyarat: itinerary, what's included, tips…"
            />
          </Field>
          <Field label="Info / details (AR)" hint="يظهر في نافذة «التفاصيل» على الموقع مع إمكانية الاستماع">
            <RichTextEditor
              ...
              placeholder="معلومات كاملة عن هذه الزيارة: البرنامج، ما تشمله الباقة، نصائح…"
            />
          </Field>
          <div className="grid grid-cols-2 gap-4">
            <ImageUpload
